...
eviction_addresses_job_yaml <- cr_build_yaml(
  steps = c(
    ...
    cr_buildstep_bash("chmod 0600 /workspace/shiny-apps-certs/client-key.pem"),
    cr_buildstep_bash("cp inst/cloudbuild/refresh_job_Dockerfile ./Dockerfile"),
    cr_buildstep_docker(
      image = "eviction-addresses-job",
      kaniko_cache = FALSE
    )
  ),
  timeout = 7200
...
cr_buildtrigger_delete("eviction-addresses-job-trigger")

cr_buildtrigger(
  build = eviction_addresses_job_build,
  name = "eviction-addresses-job-trigger",
...
res <- cr_jwt_with_httr(
  GET(str_c(url, "/refresh/documents/1000")),
  token
)
content(res)
